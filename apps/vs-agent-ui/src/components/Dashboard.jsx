...
function entryStatus(entry) {
  const state = entry.perm_state ?? 'ACTIVE'
  return { state, active: state === 'ACTIVE' && !entry.revoked && !entry.slashed }
}

/**
 * Lists the on-ledger accreditations of a DID from the declared network's
 * v3 indexer: its permission entries (/verana/perm/v1) for credential
 * schemas, with the schema title resolved for display.
 */
async function resolveDidAccreditations(did, network) {
  const base = `${network.indexerBaseUrl.replace(/\/+$/, '')}/verana`
  const list = await fetchJson(
    `${base}/perm/v1/list?did=${encodeURIComponent(did)}&response_max_size=64`,
  )

  const entries = []
  const schemaCache = new Map()
  const appUrl = APP_URLS[network.chainId]
  for (const entry of list?.permissions ?? []) {
    const schemaId = entry.schema_id
    let schemaTitle = null
    if (schemaId != null) {
      if (!schemaCache.has(schemaId)) {
        const js = await fetchJson(`${base}/cs/v1/js/${schemaId}`)
