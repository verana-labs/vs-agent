...
| `VERANA_ACCOUNT_MNEMONIC`                  | REQUIRED    | BIP-39 mnemonic for the agent's Verana account. This account is the agent's `vs_operator`: it holds only the `VSOperatorAuthorization` records granted on the agent's own Participant entries. It MUST NOT be the account that holds the Corporation's `OperatorAuthorization` — the chain forbids one account from holding both. See [doc/account-model.md](../../doc/account-model.md). |
