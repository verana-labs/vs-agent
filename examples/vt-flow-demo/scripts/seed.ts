...
async function seed() {
  ...
  if (!APPLICANT_OPERATOR) throw new Error('set DEMO_APPLICANT_OPERATOR to the applicant operator address')
  if (VALIDATOR_OPERATOR === APPLICANT_OPERATOR) {
    throw new Error(
...
seed()
  .then(result => {
    // biome-ignore lint/suspicious/noConsole: the seeded ids are this script's output
    console.log(JSON.stringify(result, null, 2))
    process.exit(0)
  })
  .catch(error => {
    // biome-ignore lint/suspicious/noConsole: the failure reason is this script's output
