...
export class EcsBootstrapService {
  ...
  private async ensureServiceIssuer(
    ...
    const onChainSchema = await indexer
      .getCredentialSchema(schema.id)
      .then(toCredentialSchema)
      .catch(() => undefined)
    ...
    if (onChainSchema.issuerOnboardingMode === ISSUER_ONBOARDING_MODE_OPEN) {
      const operatorAuths = await indexer.listOperatorAuthorizations(chain.address)
